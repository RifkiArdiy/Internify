...
@section('content')
    <div class="card card-bordered card-preview">
        <div class="card-inner">
            ...
            <form method="POST" action="{{ route('feedback-update', $feedback->feedback_id) }}" enctype="multipart/form-data"
                class="form-validate is-alter" id="feedbackForm">
                @csrf
                @method('PUT')
                ...
                <div class="row g-4">
                            ...
                            <div class="star-rating">
                                @for ($i = 5; $i >= 1; $i--)
                                    <input type="radio" id="star{{ $i }}" name="rating"
                                        value="{{ $i }}" {{ $feedback->rating == $i ? 'checked' : '' }}
                                        required />
                    ...
                    <div class="col-lg-6">
                        <div class="form-group">
                            <label class="form-label" for="judul">Judul Feedback: <span
                                    class="text-danger">*</span></label>
                            <div class="form-control-wrap">
                                <input type="text" class="form-control" id="judul" name="judul"
                                    value="{{ old('judul', $feedback->judul_feedback ?? '') }}"
                                    placeholder="Berikan judul untuk feedback anda" required>
                            </div>
                        </div>
                    </div>
