                            ...
                            <td class="nk-tb-col">
                                <div class="user-card">
                                    <div class="user-avatar bg-dark d-none d-sm-flex">
                                        @if ($magang->student->user->image)
                                            <img src="{{ Storage::url('images/users/' . $magang->student->user->image) }}"
                                                alt="{{ $magang->student->user->name }}">
                                        @else
                                            <span>{{ strtoupper(substr($magang->student->user->name, 0, 2)) }}</span>
                                        @endif
                                    </div>
                                    <div class="user-info">
                                        <span class="tb-lead">{{ $magang->student->user->name }}<span
                                                class="dot dot-success d-md-none ms-1"></span></span>
                                        <span>{{ $magang->student->user->email }}</span>
                                    </div>
                                </div>
                            </td>
